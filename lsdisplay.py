...
def _load_overrides():
    """Load display overrides from XDG config directories

    Respect XDG preference order. For example,
    ~/.config/lsdisplay/overrides.json is used before
    /etc/xdg/lsdisplay/overrides.json by default.

    Keys starting with "_" (e.g. _comment) are stripped from the result.
    """
    overrides = {}
    for xdgdir in reversed(list(xdg.BaseDirectory.load_config_paths("lsdisplay"))):
        p = os.path.join(xdgdir, "overrides.json")
        if os.path.exists(p):
            try:
                with open(p) as f:
...
def _save_overrides(overrides_dict):
    ...
    """
    config_dir = xdg.BaseDirectory.save_config_path("lsdisplay")
...
def scan_network(subnet=None):
    ...
    if updated:
        # Persist overrides to user config dir; suggest to copy to /etc/xdg.
        ...
        print(f"You can move it to /etc/xdg/lsdisplay/overrides.json for system-wide default")
    else:
