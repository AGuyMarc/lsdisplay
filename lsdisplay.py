...
# Optional dependency: pyxdg (Debian: python3-xdg) enables full XDG config-path
# semantics. When absent we fall back to hardcoded paths, so lsdisplay stays
# zero-dependency. NB: the import name "xdg" is provided by pyxdg; the unrelated
# PyPI package also named "xdg" does NOT ship xdg.BaseDirectory.
try:
    import xdg.BaseDirectory as _xdg_basedir
    _HAS_PY_XDG = True
except ImportError:
    _HAS_PY_XDG = False
...
_LEGACY_SYSTEM_DIR = "/etc/lsdisplay"   # pre-XDG location, kept as read-only fallback
_legacy_config_warned = False


def _config_load_dirs():
    """Override-file search dirs, highest priority first.

    With pyxdg: full XDG semantics (standard env vars XDG_CONFIG_HOME and
    XDG_CONFIG_DIRS, plus /etc/xdg/lsdisplay). Without it: the historical
    ~/.config/lsdisplay plus an /etc/xdg fallback. The pre-XDG /etc/lsdisplay
    is always appended last (lowest priority) so existing setups keep working.
    """
    dirs = []
    if _HAS_PY_XDG:
        dirs.extend(_xdg_basedir.load_config_paths("lsdisplay"))
    else:
        home = os.environ.get("HOME", os.path.expanduser("~"))
        dirs.append(os.path.join(home, ".config", "lsdisplay"))
        dirs.append("/etc/xdg/lsdisplay")
    dirs.append(_LEGACY_SYSTEM_DIR)
    seen, ordered = set(), []
    for d in dirs:
...
def _config_save_dir():
    """User-scope directory to write overrides.json (created if needed)."""
    if _HAS_PY_XDG:
        return _xdg_basedir.save_config_path("lsdisplay")
    home = os.environ.get("HOME", os.path.expanduser("~"))
    d = os.path.join(home, ".config", "lsdisplay")
...
def _warn_legacy_config(path):
    ...
    if not _legacy_config_warned:
        print(f"lsdisplay: note: {path} is deprecated, move it to "
              "/etc/xdg/lsdisplay/overrides.json", file=sys.stderr)
        _legacy_config_warned = True


def _load_overrides():
    """Load display overrides, merging across config dirs in XDG order.

    Files are merged lowest-priority-first so higher-priority dirs win per key.
    Keys starting with "_" (e.g. _comment) are stripped. The pre-XDG
    /etc/lsdisplay/overrides.json is read as a last-resort fallback, with a
    one-time deprecation note on stderr.
    """
    merged = {}
    for d in reversed(_config_load_dirs()):   # lowest priority first
        ...
        try:
            ...
            continue
        if d == _LEGACY_SYSTEM_DIR:
            _warn_legacy_config(p)
...
def _save_overrides(overrides_dict):
    ...
    """
    config_path = os.path.join(_config_save_dir(), "overrides.json")
...
def scan_network(subnet=None):
    ...
    if updated:
        # Persist overrides to the user config dir (no silent /etc copy).
        ...
        print("For a system-wide default, copy it to "
              "/etc/xdg/lsdisplay/overrides.json")
    else:
